...
def _plot_single(case_name: str, clean_sample: np.ndarray, triggered_sample: np.ndarray,
                 ...
                 linestyle_trigger: str = "--", max_variates: int = 10) -> None:
    """Plot all variates of a single sample with clean vs triggered overlay.
    
    Args:
        max_variates: Maximum number of variates to plot. If the sample has more,
    ...
    # Limit variates if too many
    if num_vars > max_variates:
        rng = np.random.default_rng(42)  # Fixed seed for reproducibility
        selected_vars = sorted(rng.choice(num_vars, size=max_variates, replace=False))
        clean_sample = clean_sample[:, selected_vars]
        triggered_sample = triggered_sample[:, selected_vars]
        var_labels = [f"Var {v}" for v in selected_vars]
        num_vars = max_variates
    else:
        var_labels = [f"Var {v}" for v in range(num_vars)]
