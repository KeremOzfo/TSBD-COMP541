...
def plot_latent_separability(
    ...
    """Generate a 2D projection scatter plot for clean vs poisoned samples.

    Two modes:
    1. If poison_indices provided: Compares clean samples vs poisoned samples from train set
    2. If poisoned_loader provided: Compares entire clean dataset vs poisoned dataset
    
    Args:
        model: Model to extract features from
        loader: Clean data loader (train set)
        args: Arguments with device and other configs
        trigger_model: Unused, kept for compatibility
        method: 'pca' or 'tsne'
        save_dir: Directory to save plots
        max_points: Maximum points to plot (downsampled if exceeded)
        poisoned_loader: Loader with poisoned samples (optional)
        poison_indices: Indices of poisoned samples in the training set (optional)
    ...
    # Convert poison_indices to set for fast lookup
    poison_set = set(poison_indices) if poison_indices is not None else None
    
    with torch.no_grad():
        if poison_indices is not None:
            # Mode 1: Use poison_indices to separate clean vs poisoned samples from train set
            sample_idx = 0
            for batch_x, label, padding_mask in loader:
                ...
                batch_out = model(batch_x, padding_mask, None, None)
                batch_feat = _get_feature(batch_out)
                
                # Separate based on whether sample is in poison_indices
                for i in range(batch_x.size(0)):
                    if sample_idx in poison_set:
                        bd_feats.append(batch_feat[i:i+1].detach())
                        total_bd_seen += 1
                    else:
                        clean_feats.append(batch_feat[i:i+1].detach())
                        total_clean_seen += 1
                        labels.append(label[i:i+1].detach())
                    sample_idx += 1
        
        elif poisoned_loader is not None:
            # Mode 2: Use separate clean and poisoned loaders
            ...
            # Poisoned forward passes (pre-poisoned dataset)
            ...
            raise RuntimeError("Either poison_indices or poisoned_loader is required for latent separability.")
