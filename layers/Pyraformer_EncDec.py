...
def get_mask(input_size, window_size, inner_size):
    ...
    Returns a boolean mask (seq_length, seq_length) where True denotes
    masked positions.
    """
...
def refer_points(all_sizes, window_size):
    """Gather reference indices across pyramid levels for PAM."""
...
class EncoderLayer(nn.Module):
    """Compose with self-attention and position-wise feed-forward layers."""
    ...
    def forward(self, enc_input, slf_attn_mask=None):
        """Apply self-attention with provided mask, then FFN."""
...
class Encoder(nn.Module):
    """Encoder stack with pyramid attention and bottleneck convs."""
    ...
    def forward(self, x_enc, x_mark_enc):
        """Encode input with embedding, conv bottlenecks, and attention."""
...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        """Conv1d + BN + ELU downsampling for pyramid construction."""
        x = self.downConv(x)
        x = self.norm(x)
        x = self.activation(x)
        return x
...
class Bottleneck_Construct(nn.Module):
    ...
    def forward(self, enc_input):
        """Project to bottleneck, apply convs per window, and up-project."""
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        """Apply GELU MLP with residual and optional pre/post norm."""
        residual = x
