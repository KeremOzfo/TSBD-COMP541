...
def get_frequency_modes(seq_len, modes=64, mode_select_method='random'):
    """Select frequency modes either randomly or lowest-indexed.

    Args:
        seq_len: Sequence length (L).
        modes: Number of modes to select (capped at L//2).
        mode_select_method: 'random' or other (lowest modes).

    Returns:
        Sorted list of selected frequency indices.
    """
...
class FourierBlock(nn.Module):
    ...
    def compl_mul1d(self, order, x, weights):
        """Complex multiply with real/imag handling for real tensors."""
        x_flag = True
        w_flag = True
    ...
    def forward(self, q, k, v, mask):
        """Apply Fourier mixing on queries; ignore keys/values and mask.

        Args:
            q: Query tensor (B, L, H, E).
            k, v, mask: Unused; present for API compatibility.

        Returns:
            Tuple `(x, None)` where `x` is inverse-FFT output (B, H, E, L).
        """
        # size = [B, L, H, E]
        B, L, H, E = q.shape
        x = q.permute(0, 2, 3, 1)
...
class FourierCrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        """Fourier-domain cross attention and projection back to time.

        Args:
            q, k, v: Tensors shaped (B, L, H, E).
            mask: Unused; present for API compatibility.

        Returns:
            Tuple `(out, None)` where `out` is (B, H, E, L) after iFFT.
        """
        # size = [B, L, H, E]
        B, L, H, E = q.shape
