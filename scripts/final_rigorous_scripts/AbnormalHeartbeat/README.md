# Rigorous Exploration: AbnormalHeartbeat

## Dataset Info

- **Sequence Length**: 18530
- **Master script (calls all methods)**: `run_all_methods.sh`
- **Combined script (concatenated)**: `AbnormalHeartbeat_all_methods.sh`
...
- **All methods (concatenated)**: `run_all_methods.sh` or `AbnormalHeartbeat_all_methods.sh` (same file)
...
bash run_all_methods.sh
# OR
bash AbnormalHeartbeat_all_methods.sh
```
...
head -200 AbnormalHeartbeat_all_methods.sh > AbnormalHeartbeat_subset.sh
bash AbnormalHeartbeat_subset.sh
```
