# Rigorous Exploration: PEMS-SF

## Dataset Info

- **Sequence Length**: 144
- **Master script (calls all methods)**: `run_all_methods.sh`
- **Combined script (concatenated)**: `PEMS-SF_all_methods.sh`
...
- **All methods (concatenated)**: `run_all_methods.sh` or `PEMS-SF_all_methods.sh` (same file)
...
head -200 PEMS-SF_all_methods.sh > PEMS-SF_subset.sh
bash PEMS-SF_subset.sh
```
