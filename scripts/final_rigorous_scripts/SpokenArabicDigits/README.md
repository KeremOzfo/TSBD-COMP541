# Rigorous Exploration: SpokenArabicDigits

## Dataset Info

- **Sequence Length**: 93
- **Master script (calls all methods)**: `run_all_methods.sh`
- **Combined script (concatenated)**: `SpokenArabicDigits_all_methods.sh`
...
- **All methods (concatenated)**: `run_all_methods.sh` or `SpokenArabicDigits_all_methods.sh` (same file)
...
bash run_all_methods.sh
# OR
bash SpokenArabicDigits_all_methods.sh
```
...
head -200 SpokenArabicDigits_all_methods.sh > SpokenArabicDigits_subset.sh
bash SpokenArabicDigits_subset.sh
```
