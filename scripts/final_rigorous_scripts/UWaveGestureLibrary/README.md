# Rigorous Exploration: UWaveGestureLibrary

## Dataset Info

- **Sequence Length**: 315
- **Master script (calls all methods)**: `run_all_methods.sh`
- **Combined script (concatenated)**: `UWaveGestureLibrary_all_methods.sh`
...
- **All methods (concatenated)**: `run_all_methods.sh` or `UWaveGestureLibrary_all_methods.sh` (same file)
...
bash run_all_methods.sh
# OR
bash UWaveGestureLibrary_all_methods.sh
```
...
head -200 UWaveGestureLibrary_all_methods.sh > UWaveGestureLibrary_subset.sh
bash UWaveGestureLibrary_subset.sh
```
