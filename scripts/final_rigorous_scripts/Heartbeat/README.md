# Rigorous Exploration: Heartbeat

## Dataset Info

- **Sequence Length**: 405
- **Master script (calls all methods)**: `run_all_methods.sh`
- **Combined script (concatenated)**: `Heartbeat_all_methods.sh`
...
- **All methods (concatenated)**: `run_all_methods.sh` or `Heartbeat_all_methods.sh` (same file)
...
head -200 Heartbeat_all_methods.sh > Heartbeat_subset.sh
bash Heartbeat_subset.sh
```
