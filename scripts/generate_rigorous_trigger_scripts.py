...
TRIGGER_MODELS = ['citst', 'cpatchtst']
...
# Explore different network capacities
ARCHITECTURE_CONFIGS = {
    'tiny': {
        ...
        'e_layers_bd': 1,
        'n_heads_bd': 2,
        'description': 'Minimal capacity - fast training, may underfit'
    },
    'small': {
        'd_model_bd': 32,
        'd_ff_bd': 64,
        'e_layers_bd': 1,
        'n_heads_bd': 4,
        'description': 'Small capacity - balanced speed/performance'
    },
    'medium': {
        'd_model_bd': 64,
        'd_ff_bd': 128,
        'e_layers_bd': 2,
        'n_heads_bd': 4,
        'description': 'Medium capacity - good for most datasets'
    },
    'large': {
        'd_model_bd': 128,
        'd_ff_bd': 256,
        'e_layers_bd': 2,
        'n_heads_bd': 8,
        'description': 'Large capacity - may overfit small datasets'
    },
    'deep': {
        'd_model_bd': 64,
        'd_ff_bd': 128,
        'e_layers_bd': 3,
        'n_heads_bd': 4,
        'description': 'Deeper network - more expressive but slower'
    },
    'wide': {
        'd_model_bd': 96,
        'd_ff_bd': 192,
        'e_layers_bd': 1,
        'n_heads_bd': 6,
        'description': 'Wider network - parallel processing capacity'
    }
}
...
def generate_script_command(dataset, tmodel, model, method, arch_config, opt_config, method_params, 
    ...
    cmd += f" --d_model_bd {arch_config['d_model_bd']}"
    cmd += f" --d_ff_bd {arch_config['d_ff_bd']}"
    cmd += f" --e_layers_bd {arch_config['e_layers_bd']}"
    cmd += f" --n_heads_bd {arch_config['n_heads_bd']}"
...
def main():
    ...
    base_output_dir = os.path.join(script_dir, 'rigorous_per_dataset')
    ...
    for ds in datasets:
                                ...
                                for warmup in WARMUP_EPOCHS:
                                    for batch_size in BATCH_SIZES:
                                        for poison_config in POISONING_CONFIGS:
                                            ...
                                            # Generate comment
                                            comment = f"# {tmodel} | {model} | arch={arch_name} | opt={opt_name} | "
                                            ...
                                            cmd = generate_script_command(
                                                ds, tmodel, model, method, arch_config, opt_config,
        ...
        master_script += f"# RIGOROUS PARAMETER EXPLORATION\n"
        ...
        readme += f"head -200 marksman.sh > marksman_subset.sh\n"
    ...
    for ds_sum in dataset_summaries:
        ...
        summary += f"**Location**: `rigorous_per_dataset/{ds_sum['name']}/`\n\n"
        summary += "Methods:\n"
    ...
    summary += "### Architecture Configurations (6)\n\n"
    for name, config in ARCHITECTURE_CONFIGS.items():
        summary += f"**{name}**: d_model={config['d_model_bd']}, d_ff={config['d_ff_bd']}, "
        summary += f"layers={config['e_layers_bd']}, heads={config['n_heads_bd']}\n"
    summary += "\n"
    ...
    summary += "```bash\n"
    summary += "for dataset in rigorous_per_dataset/*/; do\n"
    summary += "  cd \"$dataset\"\n"
    summary += "  bash marksman.sh\n"
    summary += "  cd ../..\n"
    summary += "done\n"
    ...
    print(f"Architecture Configs: {len(ARCHITECTURE_CONFIGS)}")
